...
(async function () {
    ...
    if (isArSessionSupported) {
        ...
        document.getElementById("sibal").addEventListener("click", window.app.arCouponPos)
    } else {
...
const url = "http://localhost:54580";
// const clientId = localStorage.getItem("clientId");

const arCouponFindEvent = async () => {
    await axios.post(url + `/calculate-service/arCoupon/client/arFindEvent/${clientId}`, {
        "clientId": clientId
        //쿠폰의 좌표 정보가 추가적으로 필요
    }).then(function (resp) {
        // setCouponPos(resp.data.result.data.couponPos);
